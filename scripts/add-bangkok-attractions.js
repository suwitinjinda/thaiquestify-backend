// scripts/add-bangkok-attractions.js
// Migration script to add Bangkok tourist attractions to MongoDB

const mongoose = require('mongoose');
const bangkokData = require('../data/tourist-attractions/bangkok');
const TouristAttraction = require('../models/TouristAttraction');

// Handle both array export and object export
const bangkokAttractions = Array.isArray(bangkokData) ? bangkokData : (bangkokData.bangkokAttractions || []);

// MongoDB connection
const MONGODB_URI = process.env.MONGODB_URI || 'mongodb://localhost:27017/thaiquestify';

async function migrateBangkokAttractions() {
  try {
    ...
    console.log(`\n📦 Processing ${bangkokAttractions.length} Bangkok attractions...\n`);

    let added = 0;
    let updated = 0;
    let skipped = 0;

    for (const attraction of bangkokAttractions) {
      try {
    ...
    console.log(`   📝 Total: ${bangkokAttractions.length}`);

    // Verify results
    const totalInDB = await TouristAttraction.countDocuments({ province: 'กรุงเทพมหานคร' });
    console.log(`\n✅ Total Bangkok attractions in database: ${totalInDB}`);
...
module.exports = migrateBangkokAttractions;
